...
namespace SortAlgoBench {
    ...
    public abstract class OrderedAlgorithms<T, TOrder>
        ...
        public static void AltTopDownMergeSort2(T[] items, int n) {
            if (!Helpers.NeedsSort_WithBoundsCheck(items, n))
                return;
            ...
            AltTopDownSplitMerge_Unsafe2(ref itemsPtr, ref Unsafe.Add(ref itemsPtr, n - 1), ref scratchPtr, ref Unsafe.Add(ref scratchPtr, n - 1), n, false);
        }

        static void AltTopDownSplitMerge_Unsafe2(ref T firstItemsPtr, ref T lastItemsPtr, ref T firstScratchPtr, ref T lastScratchPtr, int length, bool swapped) {
            if (length <= TopDownInsertionSortBatchSize) {
                if (swapped) {
                    CopyInclusiveRefRange_Unsafe(ref firstScratchPtr, ref lastScratchPtr, ref firstItemsPtr);
                }
                ref var writePtr = ref firstItemsPtr;
                ref var readPtr = ref Unsafe.Add(ref firstItemsPtr, 1);
                while (true) {//readIdx < idxEnd
                    var readValue = readPtr;
                    if (default(TOrder).LessThan(readValue, writePtr)) {
                        while (true) {
                            //default(TOrder).LessThan(readValue, writePtr) holds
                            Unsafe.Add(ref writePtr, 1) = writePtr;
                            if (Unsafe.AreSame(ref writePtr, ref firstItemsPtr)) {
                                break;
                            }
                            writePtr = ref Unsafe.Subtract(ref writePtr, 1);
                            if (!default(TOrder).LessThan(readValue, writePtr)) {
                    ...
                    if (Unsafe.AreSame(ref readPtr, ref lastItemsPtr)) {
                        break;
                    }
                    writePtr = ref readPtr;
                    readPtr = ref Unsafe.Add(ref readPtr, 1);
                ...
                var firstHalfLength = length >> 1;
                ref var middleScratchPtr = ref Unsafe.Add(ref firstScratchPtr, firstHalfLength);
                ref var justBeforeMiddltPtrA = ref Unsafe.Subtract(ref middleScratchPtr, 1);
                AltTopDownSplitMerge_Unsafe2(ref middleScratchPtr, ref lastScratchPtr, ref Unsafe.Add(ref firstItemsPtr, firstHalfLength), ref lastItemsPtr, length - firstHalfLength, !swapped);
                AltTopDownSplitMerge_Unsafe2(ref firstScratchPtr, ref justBeforeMiddltPtrA, ref firstItemsPtr, ref Unsafe.Add(ref firstItemsPtr, firstHalfLength - 1), firstHalfLength, !swapped);

                while (true) {
                    if (!default(TOrder).LessThan(middleScratchPtr, firstScratchPtr)) {
                        firstItemsPtr = firstScratchPtr;
                        firstItemsPtr = ref Unsafe.Add(ref firstItemsPtr, 1);
                        if (Unsafe.IsAddressLessThan(ref firstScratchPtr, ref justBeforeMiddltPtrA)) {
                            firstScratchPtr = ref Unsafe.Add(ref firstScratchPtr, 1);
                        } else {
                            while (true) {
                                firstItemsPtr = middleScratchPtr;
                                if (!Unsafe.AreSame(ref middleScratchPtr, ref lastScratchPtr)) {
                                    middleScratchPtr = ref Unsafe.Add(ref middleScratchPtr, 1);
                                    firstItemsPtr = ref Unsafe.Add(ref firstItemsPtr, 1);
                                    continue;
                                }
                                return;
                        ...
                        firstItemsPtr = middleScratchPtr;
                        firstItemsPtr = ref Unsafe.Add(ref firstItemsPtr, 1);
                        if (Unsafe.IsAddressLessThan(ref middleScratchPtr, ref lastScratchPtr)) {
                            middleScratchPtr = ref Unsafe.Add(ref middleScratchPtr, 1);
                        } else {
                            while (true) {
                                firstItemsPtr = firstScratchPtr;
                                if (!Unsafe.AreSame(ref firstScratchPtr, ref justBeforeMiddltPtrA)) {
                                    firstItemsPtr = ref Unsafe.Add(ref firstItemsPtr, 1);
                                    firstScratchPtr = ref Unsafe.Add(ref firstScratchPtr, 1);
                                    continue;
                                }
